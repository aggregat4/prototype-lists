...
const createStorage = () => {
  let syncState: SyncState = { clientId: "", lastServerSeq: 0 };
  let outbox: SyncOp[] = [];
  const storage: ListStorage = {
    ready: async () => {},
    clear: async () => {
      syncState = { clientId: "", lastServerSeq: 0 };
      outbox = [];
    },
    loadAllLists: async () => [],
...
test("SyncEngine flushes outbox and updates server seq", async () => {
  ...
  const fetchFn = async (url: string, init?: RequestInit) => {
    ...
    if (url.includes("/sync/push")) {
      return new Response(JSON.stringify({ serverSeq: 5 }), { status: 200 });
    }
    if (url.includes("/sync/pull")) {
      return new Response(JSON.stringify({ serverSeq: 5, ops: [] }), { status: 200 });
    }
...
test("SyncEngine applies remote ops", async () => {
  ...
  const fetchFn = async (url: string) => {
    ...
    return new Response(JSON.stringify({ serverSeq: 3 }), { status: 200 });
  };
