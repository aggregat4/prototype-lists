...
export function serializeListState(state = {}) {
    return {
        ...
        entries: serializeOrderedSetSnapshot(state.entries, mapListDataEncode),
    };
}
...
export function serializeRegistryState(state = {}) {
    return {
        version: SERIALIZATION_VERSION,
        clock: encodeTimestamp(state.clock),
        entries: serializeOrderedSetSnapshot(state.entries, mapRegistryDataEncode),
    };
}
...
export function deserializeRegistryState(encoded = {}) {
    return {
        version: encodeTimestamp(encoded.version) || SERIALIZATION_VERSION,
        clock: encodeTimestamp(encoded.clock),
        entries: deserializeOrderedSetSnapshot(encoded.entries, mapRegistryDataDecode),
    };
}
...
function serializePayload(payload) {
    ...
    Object.entries(payload).forEach(([key, value]) => {
        ...
        } else if (Array.isArray(value) && value.length > 0 && typeof value[0] === "object" && value[0]?.digit != null) {
...
export function cleanUndefinedKeys(object) {
